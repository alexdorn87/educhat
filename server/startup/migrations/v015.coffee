RocketChat.Migrations.add
	...
	up: ->
		...
		oldFilesCollection.find({'copies.files.key': {$exists: true}}).forEach (cfsRecord) ->
			...
			RocketChat.models.Messages.find({$or: [{ 'urls.url': "https://demo.Eduard_Chat/cfs/files/Files/#{cfsRecord._id}" }, { 'urls.url': "https://Eduard_Chat/cfs/files/Files/#{cfsRecord._id}" }]}).forEach (message) ->
				for urlsItem in message.urls
					if urlsItem.url is "https://demo.Eduard_Chat/cfs/files/Files/#{cfsRecord._id}" or urlsItem.url is "https://Eduard_Chat/cfs/files/Files/#{cfsRecord._id}"
						urlsItem.url = Meteor.absoluteUrl() + url
						if urlsItem.parsedUrl?.pathname?
							urlsItem.parsedUrl.pathname = "/#{url}"
						message.msg = message.msg.replace "https://demo.Eduard_Chat/cfs/files/Files/#{cfsRecord._id}", Meteor.absoluteUrl() + url
						message.msg = message.msg.replace "https://Eduard_Chat/cfs/files/Files/#{cfsRecord._id}", Meteor.absoluteUrl() + url
