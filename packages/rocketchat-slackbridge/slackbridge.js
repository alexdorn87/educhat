...
class SlackBridge {
		...
		if (data && data.data && data.data.ok === true && data.data.user && data.data.user.profile && data.data.user.profile.email) {
			...
			if (existingUser) {
				...
				Meteor.runAsUser(userData.rocketId, () => {
					...
					// Slack's is -18000 which translates to Eduard_Chat's after dividing by 3600
				...
				// Deleted users are 'inactive' users in Eduard_Chat
				if (userData.deleted) {
	...
	**/
	...
	@param [Object] user the Eduard_Chat user
	@param [Object] room the Eduard_Chat room
	...
	importMessages(rid, callback) {
		...
		if (rocketchat_room) {
			if (this.channelMap[rid]) {
				logger.class.debug('Importing messages from Slack to Eduard_Chat', this.channelMap[rid], rid);
			...
			logger.class.error('Could not find Eduard_Chat room with specified id', rid);
